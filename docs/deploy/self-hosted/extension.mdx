...
<Note>
  You can replace `0.19.4` with the `pg_search` version you wish to install and
...
curl -L "https://github.com/paradedb/paradedb/releases/download/v0.19.4/postgresql-17-pg-search_0.19.4-1PARADEDB-noble_amd64.deb" -o /tmp/pg_search.deb
sudo apt-get install -y /tmp/*.deb
```

```bash Ubuntu 22.04
# Available arch versions are amd64, arm64
curl -L "https://github.com/paradedb/paradedb/releases/download/v0.19.4/postgresql-17-pg-search_0.19.4-1PARADEDB-jammy_amd64.deb" -o /tmp/pg_search.deb
sudo apt-get install -y /tmp/*.deb
```

```bash Debian 12
# Available arch versions are amd64, arm64
curl -L "https://github.com/paradedb/paradedb/releases/download/v0.19.4/postgresql-17-pg-search_0.19.4-1PARADEDB-bookworm_amd64.deb" -o /tmp/pg_search.deb
sudo apt-get install -y /tmp/*.deb
```

```bash RHEL 9
# Available arch versions are x86_64, aarch64
curl -L "https://github.com/paradedb/paradedb/releases/download/v0.19.4/pg_search_17-0.19.4-1PARADEDB.el9.x86_64.rpm" -o /tmp/pg_search.rpm
sudo dnf install -y /tmp/*.rpm
```

```bash RHEL 8
# Available arch versions are x86_64, aarch64
curl -L "https://github.com/paradedb/paradedb/releases/download/v0.19.4/pg_search_17-0.19.4-1PARADEDB.el8.x86_64.rpm" -o /tmp/pg_search.rpm
sudo dnf install -y /tmp/*.rpm
```

```bash macOS 15 (Sequoia)
# Available arch version is arm64
curl -L "https://github.com/paradedb/paradedb/releases/download/v0.19.4/pg_search@17--0.19.4.arm64_sequoia.pkg" -o ~/Downloads/pg_search.pkg
sudo installer -pkg ~/Downloads/pg_search.pkg -target /
```

```bash macOS 14 (Sonoma)
# Available arch version is arm64
curl -L "https://github.com/paradedb/paradedb/releases/download/v0.19.4/pg_search@17--0.19.4.arm64_sonoma.pkg" -o ~/Downloads/pg_search.pkg
sudo installer -pkg ~/Downloads/pg_search.pkg -target /
```
...
**Restart** the Postgres server (full postmaster restart) for these changes to take effect. A configuration reload is not sufficient - the postmaster must be restarted to load the shared library into memory.
